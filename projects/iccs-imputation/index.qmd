...
modelo_embedding <- st$SentenceTransformer(
  "all-MiniLM-L6-v2"
)
...
embedding_iccs_impute <- function(
  ...
  emb_catalogo <- modelo$encode(
    as.list(decode_iccs$decode_iccs),
    normalize_embeddings = TRUE
  )
  
  emb_huerfanas <- modelo$encode(
    as.list(categorias_huerfanas),
    normalize_embeddings = TRUE
  )
  ...
  resultado <- tibble(
    ...
    mejor_match_decode =
      decode_iccs$decode_iccs[
        mejor_indice
      ],
    codigo_asignado =
      ifelse(
        mejor_score >= threshold,
        decode_iccs$codigo_iccs[
          mejor_indice
        ],
        NA_character_
      ),
